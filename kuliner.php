...
<body>
    ...
    <div class="container" style="margin-top: 130px !important;">
        ...
        <div class="row">
            ...
            if(isset($_GET['search'])){
                $search = $_GET['search'];
                $queryKuliner = "SELECT * FROM tb_umkm WHERE kategori = 'kuliner' AND nama LIKE '%$search%' ORDER BY nama $sort_order";
            } else {
                $queryKuliner = "SELECT * FROM tb_umkm WHERE kategori = 'kuliner' ORDER BY nama $sort_order";
            }
            
            $resultKuliner = $conn->query($queryKuliner);

            if ($resultKuliner->num_rows > 0) {
                while ($rowKuliner = $resultKuliner->fetch_assoc()) {
            ?>
                    <div class="col-md-3">
                        <a href="detail_umkm.php?id=<?php echo $rowKuliner['id_umkm']; ?>" class="card-link">
                            <div class="card mb-4">
                                <div class="card-body">
                                    <h3 class="card-title text-truncate"><?php echo $rowKuliner['nama']; ?></h3>
                                </div>
                            </div>
                        </a>
                    </div>
            <?php
                ...
                echo "Tidak ada UMKM Kuliner yang ditemukan.";
            }
    ...
    <div class="container mt-3">
        <form action="kuliner.php" method="GET">
